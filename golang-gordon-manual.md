...
在 Go 语言中，rune 是 int32 的一个别名，两者在所有方面都等价，但 rune 的主要区别在于语义上的用途。int32 是一个标准的 32 位整数类型，而 rune 类型则明确用于表示 Unicode 字符的码点(Code Point)，表示单个字符，包括汉字、表情符号等。

int32
- 类型：32 位有符号整数类型。
- 用途：用于存储常规的整数值，不特指字符含义。

rune
- 类型：是 int32 的别名，因此它也是一个 32 位整数。
- 用途：
  - 表示 Unicode 字符：当使用单引号 'a' 或 '世' 定义字符常量时，它就是一个 rune 类型的值。
  - 处理字符串：rune 类型用于表示字符串中的单个 Unicode 字符，特别是在处理非 ASCII 字符时非常有用，如中文。
  - 区分字符与整数：使用 rune 类型可以清晰地区分表示字符的整数值和普通的整数值，这提高了代码的可读性和语义性。
- 与UTF-8 编码的关系：Go 语言的字符串是以 UTF-8 编码的字节序列存储的。一个 rune 类型的值（一个Unicode 码点）在底层可能对应一个或多个字节的 UTF-8 序列。因此，要正确处理字符串中的字符，需要将字符串转换为 rune 类型的切片（[]rune(str)）进行操作。

总结

虽然 rune 和 int32 在值和内存上是相同的，但它们在编程中代表了不同的概念。int32 是底层数据类型，而 rune 是一种更抽象的、表示 Unicode 字符的类型别名。

**byte 和 uint8 的区别**

在 Go 语言中，byte 是 uint8 的一个别名，二者在本质上没有区别，都表示一个无符号的 8 位整数，可以存储 0 到 255 范围内的值。使用 byte 类型通常是为了强调数据是以原始字节的形式存在，而 uint8 更侧重于表示一个数值。

byte 与 uint8 的关系：
- 别名关系：byte 实际上是 uint8 的类型别名，编译器和运行时会将 byte 类型视作 uint8。
- 存储和内存：它们都占用 1 个字节（8 个比特位），因此在存储上完全相同。
- 值范围：byte 和 uint8 都可以表示 0 到 255 的整数值。
